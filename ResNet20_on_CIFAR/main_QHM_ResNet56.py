...
def run_exp(args):
  ...
  for epoch in range(args.epochs):
    ...
    momentum_coef = optimizer.param_groups[0]['momentum']
    nu_coef = optimizer.param_groups[0]['nu']
    train_writer.add_scalar('params/lr', lr, step)
    train_writer.add_scalar('params/momentum_coef', momentum_coef, step)

    train_writer.add_scalar('params/nu_coef', nu_coef, step)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='PyTorch ResNet110 AccSGD on CIFAR-10')
  parser.add_argument('--lr', default=1.0, type=float,
                      help='effective learning rate')
  parser.add_argument('--momentum', default=0.999, type=float,
                      help='effective learning rate')
  parser.add_argument('--nu', default=0.5, type=float,
                      help='effective learning rate')
